...
def gettingTitles(url_list):
    ...
    for i in range(len(url_list)):
        ...
        try:
            ...
            if ('link' in rss.entries[i]) == False:
                [append((rss.entries[num].title, None, rss.entries[num].enclosures[0].href)) for num in range(rss_feeds_count)]
            elif ('enclosures' in rss.entries[i]) == False:
                [append((rss.entries[num].title, rss.entries[num].link, None)) for num in range(rss_feeds_count)]
            else:
                [append((rss.entries[num].title, rss.entries[num].link, rss.entries[num].enclosures[0].href)) for num in range(rss_feeds_count)]
        except IndexError:
            pass
        try:
            if url_list[i][0] == '':
...
def generateXml(dict_url_entry, outfile):
    ...
    sorted_list = list(dict_url_entry.items())
    sorted_list.sort()
    ...
    for i in range(len(sorted_list)):
        menu = ET.SubElement(root,"menu", attrib = {"id" : sorted_list[i][0][1], "label" : str(sorted_list[i][0][0])})
        for j in range(len(sorted_list[i][1])):
            try:
                if sorted_list[i][1][j][2] == None:
                    item = ET.SubElement(menu, "item", attrib = {"label" : str(sorted_list[i][1][j][0])})
                    ...
                    podcast_menu = ET.SubElement(menu, "menu", attrib = {"id" : str(int(random.random()*10000000)) + "-menu", "label" : str(sorted_list[i][1][j][0])})
